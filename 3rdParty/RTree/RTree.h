...
public:
  ...
  struct Rect
  {
      ...
      bool isValid() const
      {
          ...
          {
              if (std::isnan(m_min[i]) || std::isnan(m_max[i]) || std::isinf(m_min[i]) || std::isinf(m_max[i]))
                  return false; // 存在NAN或无限值则无效
              if (m_min[i] > m_max[i])
                  return false;
          }
          return true;
...
public:
  ...
  // added by wangyao 2025.04.20 {
  // 获取顶层外包围盒
  Rect GetRootBoundingBox() const;
  // }
  ...
  // added by wangyao 2025.01.07 {
  int BoxSelect(osg::Polytope& polytope, bool isCross, std::function<bool(const DATATYPE&)> callback) const;
  // }
...
protected:
  ...
  // added by wangyao 2025.01.07 {
  bool BoxPick(osg::Polytope& polytope, bool isCross, Rect* a_rectB) const;
  // }
  ...
  // added by wangyao 2025.01.07 {
  bool BoxSelect(Node* a_node, osg::Polytope& polytope, bool isCross, int& a_foundCount, std::function<bool(const DATATYPE&)> callback) const;
  // }
...
// added by wangyao 2025.01.07 {
RTREE_TEMPLATE
int RTREE_QUAL::BoxSelect(osg::Polytope& polytope, bool isCross, std::function<bool(const DATATYPE&)> callback) const
{
    // NOTE: May want to return search result another way, perhaps returning the number of found elements here.

    int foundCount = 0;
    BoxSelect(m_root, polytope, isCross, foundCount, callback);
...
// added by wangyao 2025.04.20 {
RTREE_TEMPLATE
typename RTREE_QUAL::Rect RTREE_QUAL::GetRootBoundingBox() const
{
    if (m_root && m_root->m_count > 0)
    {
        return NodeCover(m_root);
    }
    else
    {
        Rect emptyRect;
...
// added by wangyao 2025.01.07 {
RTREE_TEMPLATE
bool RTREE_QUAL::BoxPick(osg::Polytope& polytope, bool isCross, Rect* a_rectB) const
{
    RTREE_ASSERT(a_rectB);

    if (NUMDIMS != 3) return false;

    osg::BoundingBox bbox;
    bbox.set(
        a_rectB->m_min[0], a_rectB->m_min[1], a_rectB->m_min[2],
        a_rectB->m_max[0], a_rectB->m_max[1], a_rectB->m_max[2]);
    return polytope.contains(bbox);
}
// }
...
// added by wangyao 2025.01.07 {
RTREE_TEMPLATE
bool RTREE_QUAL::BoxSelect(Node* a_node, osg::Polytope& polytope, bool isCross, int& a_foundCount, std::function<bool(const DATATYPE&)> callback) const
{
    RTREE_ASSERT(a_node);
    RTREE_ASSERT(a_node->m_level >= 0);
    //RTREE_ASSERT(a_rect);
    ...
    if (a_node->IsInternalNode())
    {
        ...
        for (int index = 0; index < a_node->m_count; ++index)
        {
            if (BoxPick(polytope, isCross, &a_node->m_branch[index].m_rect))
            {
                if (!BoxSelect(a_node->m_branch[index].m_child, polytope, isCross, a_foundCount, callback))
                {
                    // The callback indicated to stop searching
                    return false;
                }
